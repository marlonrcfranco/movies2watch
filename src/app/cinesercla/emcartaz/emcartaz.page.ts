...
export class EmcartazPage implements OnInit {
  ...
  goMoveDetailsPage(id: any){
    ...
    this.router.navigate(['/csmovie-details'], { state: { id: id } });
  }

  getAllActionsMovies() {
    this.movieService.getActionMovies(2).then(res =>{
      this.actionMovieList = res['results'];
    })
  }

  getAllAdventureMovies() {
    this.movieService.getAdventureMovies(3).then(res =>{
      this.adventureMovieList = res['results'];
    })
  }

  getAllAnimationMovies() {
    this.movieService.getAnimationMovies(2).then(res =>{
      this.animationMovieList = res['results'];
    })
  }
//=-
  getAllComediaMovies() {
    this.movieService.getComediaMovies(2).then(res =>{
      this.comediaMovieList = res['results'];
    })
  }

  getAllCrimeMovies() {
    this.movieService.getCrimeMovies(2).then(res =>{
      this.crimeMovieList = res['results'];
    })
  }

  getAllDocMovies() {
    this.movieService.getDocMovies(2).then(res =>{
      this.docMovieList = res['results'];
    })
  }

  getAllDramaMovies() {
    this.movieService.getDramaMovies(2).then(res =>{
      this.dramaMovieList = res['results'];
    })
  }

  getAllFamilyMovies() {
    this.movieService.getFamilyMovies(2).then(res =>{
      this.familyMovieList = res['results'];
    })
  }

  getAllFantasyMovies() {
    this.movieService.getFantasyMovies(2).then(res =>{
      this.fantasyMovieList = res['results'];
    })
  }

  getAllHistoryMovies() {
    this.movieService.getHistoryMovies(2).then(res =>{
      this.historyMovieList = res['results'];
    })
  }
  getAllTerrorMovies() {
    this.movieService.getTerrorMovies(2).then(res =>{
      this.terrorMovieList = res['results'];
    })
  }

  getAllMusicMovies() {
    this.movieService.getMusicMovies(2).then(res =>{
      this.musicMovieList = res['results'];
    })
  }

  getAllMisteryMovies() {
    this.movieService.getMisteryMovies(2).then(res =>{
      this.misteryMovieList = res['results'];
    })
  }

  getAllRomanceMovies() {
    this.movieService.getRomanceMovies(2).then(res =>{
      this.romanceMovieList = res['results'];
    })
  }

  getAllScifiMovies() {
    this.movieService.getScifiMovies(2).then(res =>{
      this.scifiMovieList = res['results'];
    })
  }

  getAllTvMovies() {
    this.movieService.getTvMovies(2).then(res =>{
      this.tvMovieList = res['results'];
    })
  }

  getAllThrillerMovies() {
    this.movieService.getThrillerMovies(2).then(res =>{
      this.thrillerMovieList = res['results'];
    })
  }

  getAllWarMovies() {
    this.movieService.getWarMovies(2).then(res =>{
      this.warMovieList = res['results'];
    })
  }

  getAllForestMovies() {
    this.movieService.getFaroestMovies(2).then(res =>{
